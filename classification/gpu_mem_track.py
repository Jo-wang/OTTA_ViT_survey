'''       
Code from: https://github.com/Oldpan/Pytorch-Memory-Utils/blob/master/gpu_mem_track.py
'''
...
class MemTracker(object):
    ...
    #             f.write(x.size(), x.dtype, np.prod(np.array(x.size()))*get_mem_space(x.dtype)/1024**2, file=file)
            
        # NOTE do not print it as this will spend more time for time.time()
        # for x in self.get_tensors():
    ...
    def do_track(self, line_info='', show_full_tensor=False):
        ...
        # with sys.stdout as f:
            ...
            wandb.log({line_info + " GPU Memory Usage":                     self.get_tensor_usage(),
                       line_info + " GPU Allocated Memory":                 self.get_allocate_usage(),
                       line_info + " GPU Cached Memory (reserved memory)":  new_cache_size,
                       line_info + " Cache increase":                       new_cache_size-self.last_cache_size
                       })
            
            f.write(f"\n@ {where_str:<50}\n"
                    f" Total Tensor Used Memory             : {self.get_tensor_usage():<7.1f}Mb\n"
                    f" Total Allocated Memory               : {self.get_allocate_usage():<7.1f}Mb\n"
                    f" Total Cached Memory (reserved memory): {new_cache_size:<7.1f}Mb\n"
